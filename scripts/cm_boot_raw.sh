...
# Determine database type by looking at local metadata for "cm_database"
cm_db_type=`curl -s http://169.254.169.254/opc/v1/instance/ | grep cm_database | cut -d ':' -f 2 | cut -d '"' -f 2`

if [ "$cm_db_type" = "postgres" ]; then 
...
" >> /etc/my.cnf
	...
	for user in `cat /etc/mysql/mysql.pw | gawk -F ':' '{print $1}'`; do
		pw=`cat /etc/mysql/mysql.pw | grep $user | cut -d ':' -f 2`
		if [ $user = "hive" ]; 
			database="metastore"
		else
			database=${user}
		fi
		/opt/cloudera/cm/schema/scm_prepare_database.sh mysql ${user} ${database} ${pw}
	done;
	#rm -fR /etc/mysql
	##
	## END MYSQL
	##
elif [ "$cm_db_type" = "oracle" ]; then 
	##
	## MAIN ORACLE INSTALL
	##
	EXECNAME="Oracle DB"
	...
	log "->ERROR - $cm_db_type NOT VALID - NO DATABASE INSTALLED.  USE postgres mysql oracle for DB METADATA NAME IN compute.tf"
fi
